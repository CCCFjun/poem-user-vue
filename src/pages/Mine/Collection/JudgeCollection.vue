  ...
  <section class="judge_collection">
    ...
    <div class="content_container">
      <div class="que_content">
        <div class="que_ques">{{item.judgeContent}}</div>
        <div
          class="que_option"
        ...
        >
          <span
            :class="{'green_op':option.value==item.judgeAnswer,'normal_op':option.value}"
          >{{option.label}}</span>
        </div>
        <div class="que_explain">
          解析：
          <span class="correct_answer">{{item.explain}}</span>
        </div>
      </div>
      <div class="que_collect">
        <img
          :src="item.isCollect == '0' ? require('../../../common/imgs/no-collect.png') : require('../../../common/imgs/yes-collect.png')"
          @click="clickCollect()"
        />
      </div>
    </div>
...
import { reqJudgeByAnswerId, reqUpdatePaperAnswerIsCollect } from "@/api/collection";
